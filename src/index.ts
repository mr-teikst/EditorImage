...
export default class ImageTool implements BlockTool {
  ...
  private set image(file: ImageSetterParam | undefined) {
    this._data.file = file || { id: "", url: "" };

    if (file && file.url) {
      this.ui.fillImage(file.url);
    } else if (file && file.id) {
      // Fetch the real URL from backend using the id
      this.fetchImageUrlById(file.id).then((url) => {
        if (url) {
  ...
  private async fetchImageUrlById(id: string): Promise<string | undefined> {
    if (!this.config.endpoints.byId) {
      console.error("No endpoint for fetching image by id");
      return undefined;
    }
    try {
      ...
      console.error("Failed to fetch image url by id", err);
      return undefined;
    }
  }
